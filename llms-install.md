...
- `includeCommitHistory`: (Optional, default: false) Include comprehensive git commit history with visual graphs, commit metadata, and optional message format assessment. Provides commit hash, author, date, message, changed files, visual commit graph (ASCII + Mermaid diagram), and git tags mapping.
...
- `analyzeCommits`: (Optional, default: false) Enable commit message format assessment and regression detection. Assesses message length, body presence, conventional format (excellent/good/fair/poor) and identifies potential bug fixes based on keywords (fix, bug, revert, etc.).
