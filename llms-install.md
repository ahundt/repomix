...
- `includeCommitHistory`: (Optional, default: false) Include comprehensive git commit history with visual graphs, commit metadata, and optional message quality analysis. Provides commit hash, author, date, message, changed files, visual commit graph (ASCII + Mermaid diagram), and git tags mapping.
...
- `analyzeCommits`: (Optional, default: false) Enable commit message quality analysis and regression detection. Assesses message quality (excellent/good/fair/poor) and identifies potential bug fixes based on keywords and patterns.
